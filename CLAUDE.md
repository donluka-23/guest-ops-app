...
- 2026-07-05: Step 7 corrected after the initial pass was rightly rejected as not actually improved. Found and fixed two real bugs the earlier round missed: (a) `--font-sans: var(--font-sans)` was a self-referential circular CSS variable present since project scaffolding, silently serif-ing the entire UI regardless of any other styling work; (b) `--background` and `--card` were within 0.01 OKLCH lightness of each other, so no shadow value could make card elevation visible. Fixed both, widened background/card contrast deliberately, bumped card shadow to `shadow-md` with a real border, added icon chips and filled status-badge pills for color presence beyond buttons. Verified with a falsifiable 5-question checklist against real Playwright screenshots of all 4 affected page types (dashboard, guests, settings, guidebook), not a subjective "looks nice" review — see "UI verification process" for why that distinction mattered (an earlier screenshot-based review had already looked at the same flat/serif output and called it done).
...
| Token | OKLCH | Hex |
|---|---|---|
| `--background` (page) | `oklch(0.965 0.008 55)` | warm cream/tan, distinctly darker/more tinted than `--card` |
| `--foreground` | `oklch(0.2 0.008 50)` | `#191513` |
| `--card` | `oklch(1 0 0)` | pure white — deliberately far from `--background` so elevation is visible |
...
**Background and card must stay visibly distinct.** An earlier version had them within 0.01 OKLCH lightness of each other (`0.985` vs `0.995`) — close enough that no shadow value made elevation visible; screenshots looked flat regardless of `shadow-sm`/`shadow-md`. Card is now pure white, background a distinctly darker/warmer tan — if either token is ever touched again, keep a real lightness gap between them, not just a shadow.

**Font bug (found 2026-07-05, present since project scaffolding):** `--font-sans` was defined as `var(--font-sans)` — a self-referential circular CSS custom property. Per spec this makes the whole declaration "guaranteed-invalid," so every element using the `font-sans`/`font-heading` utility (i.e. nearly everything, via `html { @apply font-sans }`) silently fell back to the browser's UA-default font — serif in Chrome. This is why the entire UI initially rendered in a serif face despite Geist Sans being loaded and looking "styled" in every other respect. Fixed by pointing `--font-sans` at `var(--font-geist-sans)`, the variable actually set by the `next/font` loader in `src/app/layout.tsx`. **Known residual gap:** Geist Sans has no Georgian Unicode coverage, so Georgian glyphs are always rendered by whatever font the browser substitutes for that script, regardless of this variable — not fixable by a font-family change alone, and not verified against a specific target Georgian typeface.

Component primitives (all shared, so a change here applies everywhere at once): **Button** default size `h-10`/`px-4`/`rounded-lg`/`shadow-sm`, primary variant filled solid; **Input**/**Select** trigger both `h-10` to align in form rows; **Card** `rounded-xl`, real `border border-border` + `shadow-md` (bumped from `shadow-sm`/`ring`-only once the background/card contrast fix made shadows meaningful), `20px` internal padding. Page containers: `p-4 sm:p-8`, `gap-5`–`gap-6` between sections.

**Color presence beyond buttons:** every `CardTitle` with an icon wraps it in a small chip — `<div className="flex size-7 items-center justify-center rounded-lg bg-primary/15"><Icon className="size-4 text-primary" /></div>` — rather than a bare colored icon. Status badges (guests list) are filled pills via explicit per-status classes (`STATUS_BADGE_CLASS` in `guests/page.tsx`), not `Badge`'s `outline` variant — a plain-text/outline badge doesn't read as "color present," a filled chip does.
...
- **A screenshot only helps if it's judged skeptically.** The first round of real screenshots (after setting up Playwright) was reviewed and reported as "genuinely designed, minimal SaaS-style" — but the background and card were the same white, and the whole UI was rendering in a serif font from a broken CSS variable, both clearly visible in the same screenshot already being looked at. Having the right tool didn't help because the review wasn't actually checking against concrete criteria — it was forming a favorable-sounding impression. Use a falsifiable checklist (specific yes/no questions like "is the background visibly different from the card?", not "does this look nice?") every time, and answer it as if trying to find a reason the task isn't done, not as if trying to confirm it is.
